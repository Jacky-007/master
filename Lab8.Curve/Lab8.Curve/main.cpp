...
public:
  ...
  enum DrawMode { EMPTY = 0, BRUSH, LINE, CIRCLE, ELLIPSE, POLYGON, CURVE };
...
private:
  ...
  Vector2 preview_point;

  typedef std::pair<int, int> Coord;
  std::vector<Coord> pixel_buffer;
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR pCmdLine,
  ...
  if (!win.Create(L"Lab8.Curve", WS_OVERLAPPEDWINDOW)) {
    return 0;
  }
...
void DrawWindow::OnLButtonDown(int x, int y, DWORD flags) {
  ...
  if (is_drawing) {
    if (mode == CURVE) {
      control_points.push_back(preview_point);
...
void DrawWindow::OnMouseMove(int x, int y, DWORD flags) {
  ...
  if (is_drawing) {
    ...
    case CURVE: {
      ...
      control_points.push_back(preview_point);
      draw_bezier(control_points, pixel_buffer);
    }
    }
...
void DrawWindow::OnPaint() {
  ...
  //   SetPixelV(background, pixel.first, pixel.second, RGB(170, 170, 170));
  // }
  ...
  for (int i = 0; i < control_points.size(); i++) {
    if (i > 0) {
      draw_line(control_points[i - 1].x(), control_points[i - 1].y(),
                control_points[i].x(), control_points[i].y(), pixel_buffer);
    }
    draw_circle_midpoint_bresenham(control_points[i].x(), control_points[i].y(),
                                   3, pixel_buffer);
  }
  for (auto const &pixel : pixel_buffer) {
    SetPixelV(preview, pixel.first, pixel.second, RGB(150, 150, 150));
  }
